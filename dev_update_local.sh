...
REPO="$(pwd)"
cp LICENSE LICENSE_openpgp local/python
rm -rf local/python/frogtab/local_server/static
cp -r app local/python/frogtab/local_server/static

cd "$REPO/local/python/frogtab/local_server"
rm -rf templates
mkdir templates
mv static/index.html static/icon-*.html static/help.html templates

cd "$REPO/local/python/frogtab/local_server/templates"
